...
use mau_renderer::paws::{vector, Rect, Vector};
...
impl Font {
    pub(crate) fn new(
        ...
        let face = freetype
            .new_memory_face(Rc::new(data.to_owned()), 0)
            .unwrap();
        face.set_pixel_sizes(default_size as u32, default_size as u32)
            .unwrap();
        Self {
...
impl mau_renderer::Font for Font {
...
impl<'font, 'store, 'gl> GlyphRenderer<'font, 'store, 'gl> {
    fn render_glyph(&mut self, c: char) -> Result<Glyph, Error> {
    ...
    fn get_or_render_glyph(&mut self, c: char) -> Result<&Glyph, Error> {
