...
use glam::Mat3A;
use glow::HasContext;
use mau_renderer::paws::{
    point, vector, AlignH, AlignV, Alignment, Color, LineCap, Point, Rect, Renderer, Vector,
};
use mau_renderer::{BlendMode, Font as FontTrait, Framebuffer as FramebufferTrait, RenderBackend};
...
impl RenderState {
    ...
    fn create_program(gl: &glow::Context) -> (glow::Program, Uniforms) {
      ...
      "#;
        unsafe {
            let vertex_shader =
                Self::compile_shader(gl, glow::VERTEX_SHADER, VERTEX_SHADER).unwrap();
            let fragment_shader =
            ...
            let uniforms = Uniforms {
                ...
                premultiply_alpha: gl
                    .get_uniform_location(program, "premultiply_alpha")
                    .unwrap(),
            };
    ...
    fn bind_null_texture(&mut self) {
        unsafe {
            ...
            self.gl
                .bind_texture(glow::TEXTURE_2D, Some(self.null_texture));
        }
    }
    ...
    pub(crate) fn viewport(&mut self, width: u32, height: u32) {
        self.gl_state
            .borrow_mut()
            .viewport(&self.gl, &self.uniforms, width, height);
    }
    ...
    fn apply_transform(&mut self) {
        ...
        unsafe {
            ...
            if let Some(clip_rect) = &transform.clip {
                ...
                self.gl
                    .scissor(top_left.x as i32, y as i32, width as i32, height as i32);
            } else {
...
impl Renderer for OpenGlBackend {
    ...
    fn fill(&mut self, rect: Rect, color: Color, radius: f32) {
        ...
        if radius > 0.0 {
            ...
            self.shape()
                .quad_indices(top_left, top_right, inner_top_right, inner_top_left);
            // Right edge
            ...
            self.shape()
                .quad_indices(left_upper, inner_top_left, inner_bottom_left, left_lower);
            // Corners
            self.shape()
                .arc(inner_top_left, inner_rect.top_left(), radius, PI, 1.5 * PI);
            self.shape().arc(
                inner_top_right,
    ...
    fn outline(&mut self, mut rect: Rect, color: Color, radius: f32, thickness: f32) {
        ...
        if radius > 0.0 {
            ...
            let outer_top_left = self
                .shape()
                .push_vertex(Vertex::colored(rect.top_left() - vector(d, d), color));
            let inner_top_left = self
                .shape()
                .push_vertex(Vertex::colored(rect.top_left() + vector(d, d), color));
            let outer_top_right = self
                .shape()
                .push_vertex(Vertex::colored(rect.top_right() - vector(-d, d), color));
            let inner_top_right = self
                .shape()
                .push_vertex(Vertex::colored(rect.top_right() + vector(-d, d), color));
            let outer_bottom_right = self
                .shape()
                .push_vertex(Vertex::colored(rect.bottom_right() - vector(-d, -d), color));
            let inner_bottom_right = self
                .shape()
                .push_vertex(Vertex::colored(rect.bottom_right() + vector(-d, -d), color));
            let outer_bottom_left = self
                .shape()
                .push_vertex(Vertex::colored(rect.bottom_left() - vector(d, -d), color));
            let inner_bottom_left = self
                .shape()
                .push_vertex(Vertex::colored(rect.bottom_left() + vector(d, -d), color));
            // Top edge
            self.shape().quad_indices(
    ...
    fn line(&mut self, mut a: Point, mut b: Point, color: Color, cap: LineCap, thickness: f32) {
        ...
        if cap == LineCap::Round {
            ...
            self.shape()
                .arc(a_index, a, half_thickness, angle_cw, angle_cw + PI);
            self.shape()
                .arc(b_index, b, half_thickness, angle_ccw, angle_ccw + PI);
        }
...
impl RenderBackend for OpenGlBackend {
    ...
    fn image(&mut self, rect: Rect, image: &Image) {
        ...
        unsafe {
            self.gl.active_texture(glow::TEXTURE0);
            self.gl
                .bind_texture(glow::TEXTURE_2D, Some(image.texture.texture));
            ...
            self.gl
                .texture_swizzle_mask(glow::TEXTURE_2D, &swizzle_mask);
            self.state.draw();
        }
    }
